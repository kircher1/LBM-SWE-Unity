...
public class LbmSimulation : MonoBehaviour
{
    ...
    [SerializeField]
    private float _latticeSpacingInMeters = 1.0f;

    [SerializeField]
    private int _latticeWidth = 65;

    [SerializeField]
    private int _latticeHeight = 65;

    [SerializeField]
    private float _startingHeight = 0.25f;

    [SerializeField]
    [Range(0.51f, 2.0f)]
    private float _relaxationTime = 1.5f;
    ...
    private ValueTuple<JobHandle?, Texture2D, TextureEventHandler> UpdateMaskTexture()
    {
        ...
        {
            _maskTexture =
                new Texture2D(_latticeWidth, _latticeHeight, TextureFormat.R8, mipChain: false, linear: true)
                {
